    ...
    return (
        <div className="space-y-6">
            ...
            <Card className="bg-background border-border">
                <CardHeader>
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <CardTitle className="text-foreground text-lg font-medium">{t('credit.table_title')}</CardTitle>
                        ...
                        <Table>
                            <TableHeader className="bg-muted">
                                <TableRow className="border-border hover:bg-transparent">
                                    <TableHead className="text-foreground">{t('credit.table_customer')}</TableHead>
                                    <TableHead className="text-foreground">{t('credit.table_receipt')}</TableHead>
                                    <TableHead className="text-foreground">Dat Lavant</TableHead>
                                    <TableHead className="text-foreground">Delè (Due Date)</TableHead>
                                    <TableHead className="text-right text-foreground">{t('credit.table_total')}</TableHead>
                                    <TableHead className="text-right text-foreground">{t('credit.table_advance')}</TableHead>
                                    <TableHead className="text-right text-foreground">{t('credit.table_debt')}</TableHead>
                                    <TableHead className="text-right text-foreground">{t('credit.table_actions')}</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {isLoading ? (
                                    ...
                                    filteredCredits.map((credit) => {
                                        const debt = Number(credit.total) - Number(credit.amountPaid);
                                        return (
                                            <TableRow key={credit.id} className="border-border hover:bg-muted transition-colors">
                                                <TableCell className="text-foreground font-medium">
                                                    <div className="flex flex-col">
                                                        <span>{credit.customer?.firstName} {credit.customer?.lastName}</span>
                                                        <span className="text-[10px] text-muted-foreground">{credit.customer?.phone || t('credit.no_phone')}</span>
                                                    </div>
                                                </TableCell>
                ...
                <DialogContent className="bg-background border-border text-foreground max-w-sm">
                    ...
                    {selectedCredit && (
                        <div className="space-y-4 py-4">
                            <div className="space-y-1 bg-muted p-3 rounded-lg border border-border">
                                <Label className="text-xs text-muted-foreground uppercase tracking-wider">{t('credit.table_customer')}</Label>
                                <p className="text-foreground font-medium">{selectedCredit.customer?.firstName} {selectedCredit.customer?.lastName}</p>
                                <p className="text-xs text-muted-foreground">{t('credit.table_receipt')}: {selectedCredit.receiptNumber}</p>
                            </div>

                            <div className="grid grid-cols-2 gap-3">
                                <div className="space-y-1 bg-muted p-2 rounded-md border border-border">
                                    <Label className="text-[10px] text-muted-foreground uppercase">{t('credit.table_total')}</Label>
                                    <p className="text-sm text-foreground font-mono">{Number(selectedCredit.total).toLocaleString()} {currency}</p>
                                </div>
                                <div className="space-y-1 bg-muted p-2 rounded-md border border-border">
                                    <Label className="text-[10px] text-muted-foreground uppercase">{t('credit.table_debt')}</Label>
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="amount" className="text-sm font-medium text-foreground">{t('credit.label_amount')}</Label>
                                <div className="relative">
                                    ...
                                    <Input
                                        ...
                                        className="pl-9 bg-muted border-border text-foreground text-lg h-12 focus:ring-emerald-500"
                                        autoFocus
                                    />
                                </div>
                    ...
                    <DialogFooter className="gap-2">
                        <Button
                            ...
                            className="bg-transparent border-border text-foreground hover:bg-muted flex-1"
                        >
                            {t('credit.btn_cancel')}
                        </Button>
                        <Button
