...
const PublicReceipt: React.FC = () => {
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-slate-950 text-white">
                <div className="animate-pulse flex flex-col items-center gap-4">
                    ...
                    <p className="text-slate-400">Ap chache detay resi a...</p>
                </div>
            </div>
        );
    }
    ...
    if (!sale) {
        return (
            <div className="min-h-screen flex items-center justify-center bg-slate-950 text-white p-6">
                <Card className="max-w-md w-full bg-slate-900 border-white/10 text-center p-8">
                    ...
                    <p className="text-slate-400 mb-6">Nou pa jwenn okenn tranzaksyon ki gen nimewo resi sa a (# {receiptNo}).</p>
    ...
    return (
        <div className="min-h-screen bg-slate-950 py-10 px-4 flex justify-center">
            <div className="max-w-2xl w-full space-y-6">
                <div className="flex items-center justify-between">
                    <Button variant="ghost" onClick={() => navigate('/')} className="text-slate-400 hover:text-white flex gap-2 items-center">
                ...
                <Card className="bg-slate-900 border-white/10 shadow-2xl overflow-hidden">
                    ...
                    <CardHeader className="text-center pb-2">
                        ...
                        <CardDescription className="text-slate-400 text-lg">Resi Ofisyèl # {sale.receiptNumber}</CardDescription>
                    </CardHeader>

                    <CardContent className="space-y-6">
                        <div className="grid grid-cols-2 gap-4 py-4 px-2 bg-white/5 rounded-xl border border-white/5">
                            <div className="space-y-1">
                                <p className="text-[10px] uppercase text-slate-500 font-bold tracking-widest flex items-center gap-1">
                                ...
                                {sale.items?.map((item: any, idx: number) => (
                                    <div key={idx} className="flex justify-between items-center group">
                                        <div className="flex gap-3">
                                            <div className="h-8 w-8 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-xs text-slate-400 group-hover:bg-blue-600/20 group-hover:text-blue-400 transition-colors">
                                                {item.qty}x
                                            </div>
                                            <div>
                                                <p className="text-white font-medium">{item.name}</p>
                                                <p className="text-xs text-slate-500">{item.price.toLocaleString()} {currency} pa inite</p>
                                            </div>
                                        </div>
                        ...
                        <div className="flex flex-col items-center justify-center gap-4 pt-6 text-center">
                            <div className="space-y-1">
                                <p className="text-slate-500 text-sm">Metòd Pèman: <span className="text-white font-bold">{sale.paymentMethod}</span></p>
                            ...
                            <p className="text-slate-500 text-xs italic mt-4 max-w-sm">
                                Mèsi dèske ou chwazi {sale.pos?.name || 'Magazen Kolabo'}. Resi sa a sèvi kòm prèv tranzaksyon legal.
                            </p>
                        </div>
                    </CardContent>
                    
                    <div className="bg-blue-600/10 p-4 flex justify-center border-t border-white/5">
                        <Button variant="outline" className="bg-white/5 border-white/10 text-white hover:bg-white/10 flex gap-2">
                            <Download className="h-4 w-4" /> Telechaje PDF
                        </Button>
                    </div>
                </Card>
                
                <div className="text-center text-slate-600 text-[10px] uppercase tracking-widest pb-10">
